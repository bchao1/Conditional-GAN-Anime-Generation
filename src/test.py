...
parser.add_argument('-d', '--model_dir', help = 'Folder where the trained model is saved',
                    default = './checkpoints/CGAN', type = str)
parser.add_argument('-i', '--index', help = 'Image index.', default = 1, type = int)
args = parser.parse_args()

def random(mean, std, shape):
    return torch.tensor(np.random.normal(mean, std, shape), dtype = torch.float)
...
def generate_by_attributes(model, device, latent_dim, hair_classes, eye_classes, hair_color, eye_color):
    ...
    z = random(0, 0.8,(64, latent_dim)).to(device)
    
    output = model(z, tag)
    save_image(utils.denorm(output), '{}/{}_hair_{}_eyes.png'.format(args.sample_dir, hair_mapping[hair_class], eye_mapping[eye_class]))
...
def hair_grad(model, device, latent_dim, hair_classes, eye_classes):
    ...
    save_image(utils.denorm(output), '{}/change_hair_color_{}.png'.format(args.sample_dir, args.index), nrow = hair_classes)
...
def eye_grad(model, device, latent_dim, hair_classes, eye_classes):
    ...
    output = torch.cat(img_list, 0)
    save_image(utils.denorm(output), '{}/change_eye_color_{}.png'.format(args.sample_dir, args.index), nrow = eye_classes)
...
def fix_noise(model, device, latent_dim, hair_classes, eye_classes):
    ...
    output = torch.cat(img_list, 0)
    save_image(utils.denorm(output), '{}/fix_noise_{}.png'.format(args.sample_dir, args.index), nrow = hair_classes)
...
def interpolate(model, device, latent_dim, hair_classes, eye_classes, samples = 10):
    ...
    output = torch.cat(img_list, 0)
    save_image(utils.denorm(output), '{}/interpolation_{}.png'.format(args.sample_dir, args.index), nrow = samples + 2)
...
def main():
    ...
    device = 'cuda'
    G_path = '{}/G_38000.ckpt'.format(args.model_dir)

    G = CGAN.Generator(latent_dim = latent_dim, class_dim = hair_classes + eye_classes)
